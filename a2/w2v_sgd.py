...
def stochasticGradientDescend(data,
                              U0,
                              V0,
                              W0,
                              contextFunction,
                              lossAndGradientFunction,
                              q_noise,
                              batchSize=1000,
                              epochs=1,
                              alpha=1.,
    ...
    while epoch < epochs:
        ...
        for b in range(0, len(idx), batchSize):
            ...
            ws = np.array([w for w, _ in batchData], dtype=int)
            ctx = np.stack([context for _, context in batchData]).astype(int)

            np.add.at(U, ws, -alpha * du_w)

            np.add.at(V, ctx.reshape(-1),
                      -alpha * dVt.reshape(-1, dVt.shape[-1]))
