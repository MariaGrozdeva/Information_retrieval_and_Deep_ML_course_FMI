...
if len(sys.argv) > 1 and (sys.argv[1] == 'train'
    ...
    nmt = model.LanguageModel(n_layers, n_heads, d_model, d_keys, d_values,
                              d_ff, seq_len, dropout, startToken, endToken,
                              unkToken, padToken, word2ind).to(device)
    ...
    for epoch in range(maxEpochs):
        ...
        for b in range(0, len(idx), batchSize):
            iter += 1
            batch = [
                trainCorpus[i] for i in idx[b:min(b + batchSize, len(idx))]
            ]
...
if len(sys.argv) > 3 and sys.argv[1] == 'perplexity':
    ...
    test = [[startToken] + s + [transToken] + t + [endToken]
            for (s, t) in zip(sourceTest, targetTest)]
    test = [[word2ind.get(w, word2ind[unkToken]) for w in s] for s in test]
...
if len(sys.argv) > 3 and sys.argv[1] == 'translate':
    ...
    with open(sys.argv[3], 'w', encoding='utf-8') as file:
        ...
        for s in sourceTest_tokens:
            r = nmt.generate(s + [transToken], limit=1000)
            tgt = r[r.index(transToken) + 1:-1]
            file.write(' '.join(tgt) + '\n')
            pb.tick()
